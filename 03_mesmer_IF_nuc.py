...
for index, row in df.iterrows():
    if row['dapimax'] > 10 :
        ...
        filename=imgpath.split('/')[-1].replace('_y_','_y_0').replace('y_010','y_10').replace('y_011','y_11')+"_mask.tif"
        
        # Load the images
        im1 = imread(imgpath)[:,:,0] # input DAPI channel  
        im2 = imread(imgpath)[:,:,0] # input DAPI channel  

        # Combined together and expand to 4D 
        im = np.stack((im1, im2), axis=-1)
        im = np.expand_dims(im,0)
        
        labeled_image = app.predict(im, image_mpp=0.325, compartment = 'nuclear') # input your image's resolution, ex. CRC: 0.325um/pix  
        mask=labeled_image[0,:,:,0] 
        Image.fromarray(mask).save(f'IFtilemask_mesmer_nuc/{filename}')
        print(filename)
    else:
        imgpath='IFtile/'+ row['tile'].split('/')[-1].replace('_mask.png','')
        filename=imgpath.split('/')[-1].replace('_y_','_y_0').replace('y_010','y_10').replace('y_011','y_11')+"_mask.tif"
        ...
        Image.fromarray(empty_img).save(f'IFtilemask_mesmer_nuc/{filename}')
        print(filename)
